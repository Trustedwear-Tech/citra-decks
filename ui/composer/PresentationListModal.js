...
// PresentationListModal.js - Full-screen modal to browse, load, and manage saved presentations
...
const PresentationListModal = ({
    ...
    const fetchPresentations = useCallback(async (isLoadMore = false) => {
        ...
        try {
            ...
            // Fetch ALL user presentations (user-level, not workspace-specific)
            let url = `${apiConfig.API_URL}/presentation/list?skip=${currentOffset}&limit=${LIMIT}&all_workspaces=true`;
            // Remove team_id filter to get all user presentations across all workspaces
            ...
            if (response.ok) {
                ...
                const newItems = data.presentations || [];
                const myItems = newItems.filter(item => !item.isShared);

                if (isLoadMore) {
                    setPresentations(prev => [...prev, ...myItems]);
                } else {
                    setPresentations(myItems);
                }
            ...
            if (!isLoadMore) setError('Failed to load presentations');
    ...
    const fetchSharedPresentations = useCallback(async () => {
        try {
            setIsLoading(true);
            setError(null);
            console.log('Fetching ALL shared presentations (user-level, all workspaces) for', userDeviceId);

            // Fetch all shared presentations via centralized sharing endpoint
            const token = await AsyncStorage.getItem('@auth_token');
            const response = await fetch(
                `${apiConfig.API_URL}/api/sharing/my-shared/presentation`,
                {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json',
    ...
    useEffect(() => {
        if (visible) {
            if (activeTab === 'my') {
                ...
                setOffset(0);
                setHasMore(true);
                fetchPresentations(false);
            } else {
                fetchSharedPresentations();
            }
        }
    }, [visible, activeTab]);
    ...
    const handleLoadMore = () => {
        ...
        if (activeTab === 'my' && !isLoading && !isFetchingMore && hasMore && !error) {
            fetchPresentations(true);
        }
    };

    const handleLoadPresentation = async (presentationId) => {
        console.log('[PRESENTATION_LIST_MODAL] Loading presentation ID:', presentationId);
        try {
            setIsLoading(true);
            const token = await AsyncStorage.getItem('@auth_token');

            const response = await fetch(
                `${apiConfig.API_URL}/presentation/load/${presentationId}`,
                {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json'
            ...
            if (response.ok) {
                const data = await response.json();
                console.log('[PRESENTATION_LIST_MODAL] Fetched presentation data:', data);

                // Call onLoadPresentation with the presentation data
                onLoadPresentation(data.presentation || data);
                onClose();
            } else {
    ...
    const handleDeletePresentation = async (presentationId, title) => {
        const confirmDelete = async () => {
            try {
                const token = await AsyncStorage.getItem('@auth_token');
                const response = await fetch(
                    `${apiConfig.API_URL}/presentation/${presentationId}`,
                    {
                        method: 'DELETE',
                        headers: {
                            'Authorization': `Bearer ${token}`
                ...
                if (Platform.OS === 'web') {
                    window.alert('Failed to delete presentation: ' + err.message);
                } else {
                    Alert.alert('Error', 'Failed to delete presentation');
                }
            } finally {
    ...
    // Render presentation card with thumbnail
    const renderPresentationCard = (presentation) => {
        const hasThumbnail = presentation.thumbnail && typeof presentation.thumbnail === 'string';
        ...
        return (
            <TouchableOpacity
                ...
                onPress={() => handleLoadPresentation(presentation.id)}
                activeOpacity={0.8}
            >
                {/* Thumbnail Area */}
                <View style={[styles.cardThumbnail, { height: thumbnailHeight }]}>
                    {hasThumbnail && !imageLoadErrors[presentation.id] ? (
                        <LazyThumbnail
                            uri={presentation.thumbnail}
                            style={styles.thumbnailImage}
                            resizeMode="cover"
                            onError={() => handleImageError(presentation.id)}
                            placeholder={
                    ...
                    {/* Delete button overlay - hidden for shared presentations */}
                    ...
                    {presentation.slide_count && (
                        <View style={styles.slideCountBadge}>
                            <Text style={styles.slideCountText}>{presentation.slide_count} slides</Text>
                        </View>
                    )}
                </View>
                ...
                <View style={styles.cardInfo}>
                    ...
                    {presentation.goal && (
                        <Text style={styles.cardGoal} numberOfLines={1}>
                            {(() => {
                                if (!presentation.goal) return '';
                                if (typeof presentation.goal === 'string') return presentation.goal;
                                if (typeof presentation.goal === 'object') {
                                    const p = presentation.goal;
                                    if (p.purpose && typeof p.purpose === 'string') return p.purpose;
                                    if (p.presentationType && typeof p.presentationType === 'string') return p.presentationType;
                                    return 'Presentation';
                                }
                                return 'Presentation';
                            })()}
                    ...
                    {presentation.workspace_name && presentation.isShared && (
                        <View style={styles.workspaceBadge}>
                            <Ionicons name="briefcase-outline" size={11} color="#6B7280" />
                            <Text style={styles.workspaceBadgeText}>{presentation.workspace_name}</Text>
                        </View>
                    )}
                </View>
            </TouchableOpacity>
        ...
        <View style={[styles.fullScreenContainer, { backgroundColor: safeTheme.background }]}>
            ...
            {isLoading ? (
                <View style={styles.fullScreenCentered}>
                    ...
                    <Text style={[styles.loadingText, { color: safeTheme.text }]}>Loading presentations...</Text>
                </View>
            ) : error ? (
                <View style={styles.fullScreenCentered}>
                    <MaterialIcons name="error-outline" size={64} color="#f44336" />
                    <Text style={styles.errorText}>{error}</Text>
                    <TouchableOpacity onPress={fetchPresentations} style={styles.retryBtn}>
                ...
                <View style={{ flex: 1, paddingHorizontal: isMobile ? 12 : 40 }}>
                    <FlatList
                        data={activeTab === 'my'
                            ? [{ id: 'create_new' }, ...presentations]
                            : sharedPresentations}
                        renderItem={({ item }) => {
                            if (item.id === 'create_new') {
                                return renderCreateNewCard();
                            }
                            return renderPresentationCard(item);
                        }}
                        ...
                        ListEmptyComponent={
                            <View style={[styles.emptyStateInline, { width: isMobile ? '100%' : Math.min(cardWidth * 2 + 20, screenDimensions.width - 80), maxWidth: '100%' }]}>
                                <Text style={[styles.emptyText, { color: safeTheme.text, textAlign: 'center' }]}>
                                    {activeTab === 'my' ? 'No saved presentations yet' : 'No presentations shared with you'}
                                </Text>
                                <Text style={[styles.emptySubtext, { textAlign: 'center' }]}>
                                    {activeTab === 'my' ? 'Click "Create New Presentation" to get started' : 'Presentations shared by others will appear here'}
                                </Text>
                            </View>
                        }
                    />
...
export default PresentationListModal;
