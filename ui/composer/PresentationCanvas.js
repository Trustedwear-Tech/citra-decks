// PresentationCanvas.js - Canvas editor component for slide editing using Fabric.js
...
// ===== FIX: Intercept 'alphabetical' textBaseline at the CanvasRenderingContext2D level =====
// Fabric.js 5.3.1 internally uses the string 'alphabetical' in its rendering methods.
// Prototype patches on Fabric classes don't intercept these internal calls.
// Instead, we intercept at the browser API level: any attempt to set textBaseline='alphabetical'
// is silently corrected to 'alphabetic'.
if (typeof window !== 'undefined' && typeof CanvasRenderingContext2D !== 'undefined') {
  try {
    ...
    // Fallback: suppress console warnings if property interception fails
    const originalWarn = console.warn;
    const originalError = console.error;
    const shouldSuppress = (args) => {
      const msg = args[0];
      return typeof msg === 'string' && msg.includes('alphabetical') && msg.includes('CanvasTextBaseline');
    };
...
const loadFabricFromCDN = () => {
  return new Promise((resolve, reject) => {
    ...
    script.onload = async () => {
      ...
      // Note: textBaseline 'alphabetical' → 'alphabetic' fix is handled globally via
      // Object.defineProperty interceptor on CanvasRenderingContext2D.prototype (above).
      // No per-prototype patching needed here.
...
import { mapIconToPath, getIconSVG, mapIconToPathAsync, cacheIconSVG, ICON_PATHS } from './utils/iconMapper';
...
import ShapesPickerModal from './ShapesPickerModal';
...
import FormsButtonModal from './FormsButtonModal';
import InlineVideoOverlay from './InlineVideoOverlay';

import { ShareButton } from '../ShareManager';
import { FabricYjsBinder } from './utils/FabricYjsBinder';
import { preprocessSvgForFabric } from './utils/svgFabricPrep';
...
// A "card" in legacy / free-form decks is rarely a `card` element — the
...
// layer over the slide background.
...
/**
 * PresentationCanvas - A canvas editor for creating and editing presentation slides
 * Uses Fabric.js for web platform with fallback for native
 */
const PresentationCanvas = forwardRef(({
  theme,
  slide,
  isActiveSlide = false,
  ...
  onSelectElement,
  onUpdateElement,
  onUpdateSlideBackground, // New prop for updating slide background color
  onDeleteElement,
  ...
  presentationId, // New prop for sharing
  onUpdateTitle,
  onSave,
  onExport,
  onPresent,
  ...
  const [fontSize, setFontSize] = useState(presentationStyle?.textStyles?.body?.fontSize || 16);
  const [lineHeight, setLineHeight] = useState(1.4);

  const [selectedObjectInfo, setSelectedObjectInfo] = useState(null); // { type, fill, stroke, fontSize }

  // Inline Floating Toolbar State (for character-level formatting)
  const [inlineToolbar, setInlineToolbar] = useState({ visible: false, x: 0, y: 0, styles: {} });
  const inlineToolbarRef = useRef({ visible: false }); // ref for event handlers (avoids stale closure)
  ...
  const isEditingTextRef = useRef(false); // Track if text is being edited to prevent sync overwrites
  const recentlyEditedElementRef = useRef(null); // Track element ID that was just edited
  const onElementSelectionChangeRef = useRef(onElementSelectionChange);
  const recentlyModifiedElementsRef = useRef(new Map()); // Track elements modified via toolbar (color, font, etc.) - Map<elementId, timestamp>
  ...
  // Undo/Redo history - PER-SLIDE system using Map
  // Key: slideId, Value: { states: [], index: number, lastSaved: string }
  const slideHistoriesRef = useRef(new Map());
  const currentSlideIdRef = useRef(null); // Track current slide for history operations
  const hasInitialSaveRef = useRef(false); // Track if initial state was saved for current slide
  ...
  const historyDebounceMapRef = useRef(new Map()); // Debounce timers per slide
  const preEditSnapshotRef = useRef(null); // Snapshot taken BEFORE an edit starts
  const prevElementsMapRef = useRef(new Map()); // Track last-known elements per slide to avoid cross-slide pollution
  const lastExternalSessionRef = useRef(new Map()); // slideId -> agent-turn session id that last pushed history (coalescing)

  // Helper to get or create history for a slide
  const getSlideHistory = useCallback((slideId) => {
    if (!slideId) return null;
    if (!slideHistoriesRef.current.has(slideId)) {
      slideHistoriesRef.current.set(slideId, { states: [], index: -1, lastSaved: null, undoRedoActive: false });
    } else {
      const existing = slideHistoriesRef.current.get(slideId);
      if (existing.undoRedoActive === undefined) {
        existing.undoRedoActive = false;
      }
    }
    return slideHistoriesRef.current.get(slideId);
  }, []);

  // Collect the latest slide snapshot (elements + background) directly from Fabric objects to avoid stale React state
  const collectCurrentSlideState = useCallback((slideId) => {
    if (!slideId) return null;

    const canvas = fabricCanvasRef.current;
    const baseElements = slideRef.current?.elements || [];
    const backgroundColor = slideRef.current?.backgroundColor;
    ...
    const merged = baseElements.map((el) => {
      ...
      // This prevents capturing render defaults (from presentation style) as explicit values
      if ((obj.type === 'textbox' || obj.type === 'i-text')) {
        ...
        // Capture per-character inline styles for undo/redo persistence
  ...
  // Helpers for per-slide undo/redo activity
  const isUndoRedoActiveFor = useCallback((slideId) => {
    if (!slideId) return false;
    const history = getSlideHistory(slideId);
    return !!history?.undoRedoActive;
  }, [getSlideHistory]);

  const setUndoRedoActiveFor = useCallback((slideId, active) => {
    const history = getSlideHistory(slideId);
    if (history) {
  ...
  useEffect(() => {
    ...
    // Bind
    binder.setCurrentSlideId(slide?.id);
    binder.bind();

    return () => {
      binder.unbind();
    };
  }, [awareness, slide?.id, isCanvasReady]); // Re-bind if slide changes

  // Sync toolbar state with selection
  useEffect(() => {
    if (selectedElementId && slide?.elements) {
      ...
      if (element) {
        ...
        // SMART PROTECTION CLEARING: If this element was recently modified,
        // check if React state now matches what we set - if so, clear protection
        if (recentlyModifiedElementsRef.current.has(element.id)) {
          const canvas = fabricCanvasRef.current;
          const fabricObj = canvas?.getObjects().find(o => o.elementId === element.id);
          if (fabricObj) {
            const fabricFill = fabricObj.fill;
            const reactFill = element.color || element.fill;
            // If React state now matches Fabric, React has caught up - clear protection
            if (fabricFill === reactFill) {
              console.log(`✅ [CANVAS] React state synced for ${element.id}, clearing protection`);
              recentlyModifiedElementsRef.current.delete(element.id);
            }
          }
        }
      }
  ...
  // Track slide changes and initialize per-slide history
  // Also store per-slide previous elements to avoid cross-slide pollution
  useEffect(() => {
    if (!slide?.id) return;

    // Detect slide change
    if (slide.id !== currentSlideIdRef.current) {
      // console.log('🔄 [HISTORY] Slide changed from', currentSlideIdRef.current, 'to', slide.id);
      currentSlideIdRef.current = slide.id;
      hasInitialSaveRef.current = false;
      preEditSnapshotRef.current = null;
      const history = getSlideHistory(slide.id);
      if (history) history.undoRedoActive = false;
      // Reset per-slide prevElements immediately to avoid borrowing previous slide state
      prevElementsMapRef.current.set(slide.id, JSON.stringify({
        elements: slide.elements || [],
        backgroundColor: slide.backgroundColor,
      }));

      // Clear Fabric canvas selection when slide changes to avoid ghost selection overlay
      if (fabricCanvasRef.current) {
        // console.log('🧹 [CANVAS] Clearing selection on slide change');
        fabricCanvasRef.current.discardActiveObject();
        fabricCanvasRef.current.requestRenderAll();
      }

      // Clear any pending debounce for the previous slide
    ...
    const history = getSlideHistory(slide.id);
    // Seed history from slide snapshot (elements + background), not the current Fabric canvas
    const initialState = {
    ...
    // Initialize history for this slide if needed
    if (!hasInitialSaveRef.current) {
      if (history && history.states.length === 0) {
        // console.log('📸 [HISTORY] Saving initial state for slide', slide.id, 'with', initialState.elements?.length || 0, 'elements');
    ...
    if (history && history.states.length === 1 && history.index === 0) {
      ...
      if (savedElements.length === 0 && (initialState.elements?.length || 0) > 0) {
        ...
        history.lastSaved = initialStateStr;
        // console.log('📸 [HISTORY] Backfilled initial state for slide', slide.id, 'with', initialState.elements?.length || 0, 'elements');
      }
    }

    // Initialize prevElements for this slide to current elements
    if (history && initialStateStr && (!prevElementsMapRef.current.has(slide.id) || prevElementsMapRef.current.get(slide.id) === '[]')) {
      prevElementsMapRef.current.set(slide.id, initialStateStr);
    }
  }, [isCanvasReady, slide?.id, slide?.elements, getSlideHistory, collectCurrentSlideState]);

  // Detect external changes (AI edits) and save to history
  // This catches changes that bypass canvas event handlers
  // Uses per-slide tracking to avoid cross-slide pollution
  useEffect(() => {
    if (!slide?.id || !slide?.elements || isUndoRedoActiveFor(slide.id)) return;

    try {
      const currentSnapshot = { elements: slide.elements || [], backgroundColor: slide.backgroundColor };
      const currentStr = JSON.stringify(currentSnapshot);

      // Get previous elements for THIS SPECIFIC SLIDE
      const prevStr = prevElementsMapRef.current.get(slide.id);

      // Skip if this is the initial load for this slide
      if (prevStr === undefined) {
        prevElementsMapRef.current.set(slide.id, currentStr);
        return;
      }
      ...
      if (currentStr !== prevStr && currentStr !== history.lastSaved) {
        ...
        // console.log('🤖 [HISTORY] External change detected for slide', slide.id, '(AI edit?)');

        // ATOMIC AGENT TURN: if this change belongs to the SAME agent turn that
        // already pushed history for this slide (and nothing else was saved in
        ...
        const sameAgentTurn = externalEditSessionId
          && lastExternalSessionRef.current.get(slide.id) === externalEditSessionId
          && history.index >= 0
        ...
        if (sameAgentTurn) {
          ...
          // console.log('💾 [HISTORY] Coalesced agent-turn state for slide', slide.id, 'index:', history.index);
        } else {
          // Parse the previous state for THIS slide
          ...
          if (prevStateStr !== history.lastSaved) {
            ...
            history.index = history.states.length - 1;
            // console.log('💾 [HISTORY] Saved pre-AI state for slide', slide.id, 'index:', history.index);
          }
          ...
          history.index = history.states.length - 1;
          history.lastSaved = currentStr;
          // console.log('💾 [HISTORY] Saved post-AI state for slide', slide.id, 'index:', history.index);

          // Remember which agent turn (if any) produced the top of this history.
          lastExternalSessionRef.current.set(slide.id, externalEditSessionId || null);
      ...
      // Always update the previous elements for THIS slide
      prevElementsMapRef.current.set(slide.id, currentStr);
    } catch (e) {
  ...
  }, [slide?.id, slide?.elements, getSlideHistory, externalEditSessionId]);
  ...
  useEffect(() => {
    ...
    const handleShortcuts = (e) => {
      ...
      // Select All: Ctrl+A / Cmd+A - select all canvas objects on this slide
      if ((e.ctrlKey || e.metaKey) && (e.key === 'a' || e.key === 'A')) {
        console.log('🔲 [KEYBOARD] Ctrl+A pressed - selecting all selectable canvas objects on current slide');
        e.stopPropagation();
      ...
      if (e.key === 'Delete' || e.key === 'Backspace') {
        ...
        if (activeObj && activeObj.type === 'activeSelection') {
          ...
          // Store slide ID now to avoid stale reference during deletion
          ...
          if (elementIds.length > 0 && slideId && batchDeleteHandler) {
            ...
            batchDeleteHandler(slideId, elementIds);
        ...
        // Handle single object
          ...
          if (elementId && slideRef.current && handlersRef.current.onDeleteElement) {
            handlersRef.current.onDeleteElement(slideRef.current.id, elementId);
        ...
        if (activeObj && onCopyElements) {
          ...
          if (activeObj.type === 'activeSelection') {
            ...
            selectedObjects.forEach(obj => {
              if (obj.elementId && slideRef.current?.elements) {
                const element = slideRef.current.elements.find(el => el.id === obj.elementId);
                if (element) selectedElements.push(element);
              }
            });
          } else if (activeObj.elementId && slideRef.current?.elements) {
            // Single selection
            const element = slideRef.current.elements.find(el => el.id === activeObj.elementId);
            if (element) selectedElements.push(element);
          }

          if (selectedElements.length > 0) {
            onCopyElements(selectedElements, slideRef.current?.id);
        ...
        if (activeObj && onCopyElements) {
          ...
          if (selectedElements.length > 0) {
            ...
            if (activeObj.type === 'activeSelection') {
              ...
              canvas.discardActiveObject();
              canvas.requestRenderAll();
              if (onDeleteMultipleElements && slideId) {
                onDeleteMultipleElements(slideId, elementIds);
              }
            } else if (activeObj.elementId && handlersRef.current.onDeleteElement && slideId) {
              handlersRef.current.onDeleteElement(slideId, activeObj.elementId);
            }
      ...
      if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key) && activeObj) {
        ...
        if (activeObj.type === 'activeSelection') {
          ...
          activeObj.getObjects().forEach(obj => {
            ...
            if (obj.elementId && slideRef.current && handlersRef.current.onUpdateElement) {
              const scale = scaleRef.current || 1;
              handlersRef.current.onUpdateElement(slideRef.current.id, obj.elementId, {
          ...
          if (activeObj.elementId && slideRef.current && handlersRef.current.onUpdateElement) {
            const scale = scaleRef.current || 1;
            handlersRef.current.onUpdateElement(slideRef.current.id, activeObj.elementId, {
  ...
  useEffect(() => {
    ...
    const handlePaste = (e) => {
      // Only handle paste on the currently active slide canvas
        ...
        if (items[i].type.indexOf('image') !== -1) {
          ...
          reader.onload = (event) => {
            ...
            imgObj.onload = () => {
              ...
              fabricImg.set({
                left: SLIDE_WIDTH / 2 - (fabricImg.width * scale) / 2,
                top: SLIDE_HEIGHT / 2 - (fabricImg.height * scale) / 2,
                scaleX: scale,
                scaleY: scale,
              });
              ...
              if (slide?.id) {
                const maxZ = slide.elements?.length > 0
                  ? Math.max(...slide.elements.map(e => parseInt(e.zIndex) || 0)) + 1
                  : 1;
                onAddElement(slide.id, 'image', {
  ...
  }, [isActiveSlide, slide?.id, fillColor, fontSize, onAddElement, onPasteElements]);

  // SINGLE ACTIVE SELECTION ACROSS SLIDES. Every slide mounts its own Fabric
  // canvas, and Fabric keeps a canvas's selection when the user clicks a
  // DIFFERENT slide's canvas — the stale off-screen selection then ALSO receives
  // document-level keyboard shortcuts (Delete / Ctrl+Z / arrow-nudge), hitting
  // two slides with one keystroke. PowerPoint has exactly one selection at a
  ...
  useEffect(() => {
    ...
    const announceFocus = () => {
      try {
        window.dispatchEvent(new CustomEvent('citra-canvas-focus', { detail: { canvasId: slideRef.current?.id } }));
      } catch { /* CustomEvent unavailable — nothing to do */ }
    };
    const onFocusElsewhere = (e) => {
      const focusedId = e?.detail?.canvasId;
      if (!focusedId || focusedId === slideRef.current?.id) return;
  ...
  useEffect(() => {
    ...
    const initCanvas = async () => {
      try {
        ...
        // Note: textBaseline 'alphabetical' → 'alphabetic' is handled globally via
        // Object.defineProperty interceptor — no per-canvas patching needed.
        ...
        canvas.on('mouse:down', (e) => {
          if (formatPainterActiveRef.current && formatPainterDataRef.current && e.target && e.target.elementId) {
            ...
            if (onFormatPainterApplyRef.current) {
              const targetElement = slideRef.current?.elements?.find(el => el.id === targetElementId);
              if (targetElement) {
        ...
        canvas.on('selection:created', (e) => {
          ...
          if (selectedElementIds.length > 0) {
            ...
            if (formatPainterActiveRef.current && formatPainterDataRef.current && onFormatPainterApplyRef.current) {
              const targetElement = slideRef.current?.elements?.find(el => el.id === selectedElementIds[0]);
              if (targetElement) {
        ...
        canvas.on('object:modified', (e) => {
          ...
          if (obj?.elementId && slideRef.current) {
            ...
            if (handlersRef.current.onUpdateElement) {
              handlersRef.current.onUpdateElement(slideRef.current.id, obj.elementId, updates);
              handlersRef.current.saveToHistory(); // Commit pre-edit snapshot + save new state
            }
          } else if (obj?.type === 'activeSelection' && slideRef.current) {
            ...
            if (children.length > 0 && handlersRef.current.onUpdateElement) {
              ...
              children.forEach(child => {
                ...
                handlersRef.current.onUpdateElement(slideRef.current.id, child.elementId, childUpdates);
              });
              handlersRef.current.saveToHistory();
            }
          }
        ...
        canvas.on('text:editing:entered', (e) => {
          console.log('✏️ [CANVAS] Text editing ENTERED - blocking sync');
          isEditingTextRef.current = true;
          if (e.target?.elementId) {
            recentlyEditedElementRef.current = e.target.elementId;
          }
          if (handlersRef.current.capturePreEditSnapshot) {
            handlersRef.current.capturePreEditSnapshot();
          }
          ...
          if (textObj) {
            const selectionHandler = () => {
              ...
              if (start !== end) {
                ...
                // Read current styles at selection
                const selStyles = textObj.getSelectionStyles ? textObj.getSelectionStyles(start, end) : [];
                const firstStyle = selStyles.length > 0 ? selStyles[0] : {};
                const currentScale = scaleRef.current || 1;

                // Compute position: get the textbox bounding rect on screen
                const boundingRect = textObj.getBoundingRect();
                // Position toolbar centered above the textbox top
                const posX = boundingRect.left + boundingRect.width / 2;
                const posY = boundingRect.top;

                const resolvedStyles = {
                  fontWeight: firstStyle.fontWeight || textObj.fontWeight || 'normal',
                  fontStyle: firstStyle.fontStyle || textObj.fontStyle || 'normal',
                  underline: firstStyle.underline !== undefined ? firstStyle.underline : (textObj.underline || false),
                  fill: firstStyle.fill || textObj.fill || '#000000',
                  textBackgroundColor: firstStyle.textBackgroundColor || '',
                  fontFamily: firstStyle.fontFamily || textObj.fontFamily || 'Inter',
                  fontSize: (firstStyle.fontSize || textObj.fontSize || 16) / currentScale,
                };

                if (handlersRef.current.setInlineToolbar) {
                  handlersRef.current.setInlineToolbar({ visible: true, x: posX, y: posY, styles: resolvedStyles });
                }
              } else {
                // Cursor only — hide toolbar
                if (handlersRef.current.setInlineToolbar) {
                  handlersRef.current.setInlineToolbar({ visible: false, x: 0, y: 0, styles: {} });
                }
              }
            };
            textObj.on('selection:changed', selectionHandler);
            textObj._inlineSelectionHandler = selectionHandler; // Store for cleanup
          }
        });
        ...
        canvas.on('text:changed', (e) => {
          const obj = e.target;
          if (obj?.elementId && slideRef.current && handlersRef.current.onUpdateElement) {
            handlersRef.current.onUpdateElement(slideRef.current.id, obj.elementId, { content: obj.text });
        ...
        canvas.on('text:editing:exited', (e) => {
          ...
          console.log('✏️ [CANVAS] Text editing EXITED');
          ...
          if (textObject && textObject._inlineSelectionHandler) {
            textObject.off('selection:changed', textObject._inlineSelectionHandler);
            delete textObject._inlineSelectionHandler;
          }

          // Persist per-character styles before exiting
          if (textObject && textObject.elementId && slideRef.current && handlersRef.current.onUpdateElement) {
            const elementId = textObject.elementId;
            const newText = textObject.text;
            const charStyles = textObject.styles || {};
            console.log(`✏️ [CANVAS] Committing text on exit: "${newText?.substring(0, 50)}..."`);
            handlersRef.current.onUpdateElement(slideRef.current.id, elementId, {
              content: newText,
              styles: JSON.parse(JSON.stringify(charStyles)),
            });
          }
          ...
          // Release the editing lock after state has time to propagate
          setTimeout(() => {
            console.log('✏️ [CANVAS] Text editing lock RELEASED');
            isEditingTextRef.current = false;
            recentlyEditedElementRef.current = null;
          }, 200);
        });
        ...
        canvas.on('mouse:down', (opt) => {
          ...
          if (opt.target && lastClickTarget === opt.target && (now - lastClickTime) < 500) {
            if (opt.target.type === 'chart' && opt.target.elementId) {
              ...
              const element = currentSlide?.elements?.find(e => e.id === opt.target.elementId);
              ...
              const cs = slideRef.current;
              const el = cs?.elements?.find(e => e.id === opt.target.elementId);
                ...
                if (cfg) {
                  ...
                  if (row >= 0 && row < cfg.rows && col >= 0 && col < cfg.cols) {
                    ...
                    tempInput.on('editing:exited', () => {
                      ...
                      // Persist changes
                      if (handlersRef.current.onUpdateElement && slideRef.current) {
                        handlersRef.current.onUpdateElement(slideRef.current.id, table.elementId, {
        ...
        canvas.on('mouse:dblclick', (opt) => {
          ...
          if (opt.target && opt.target.type === 'chart' && opt.target.elementId) {
            ...
            // Use slideRef to avoid stale closure (slide might have changed since listener init)
            const currentSlide = slideRef.current;
            const element = currentSlide?.elements?.find(e => e.id === opt.target.elementId);
        ...
        canvas.on('mouse:up', (opt) => {
          ...
          if (opt.isClick && opt.target) {
            ...
            if (isVideoOverlay || isEmbedOverlay || (t.type === 'group' && (t.videoType || t.embedType))) {
              ...
              const currentSlide = slideRef.current;
              const element = currentSlide?.elements?.find(e => e.id === t.elementId);
    ...
    return () => {
      ...
      if (fabricCanvasRef.current) {
        ...
        // with 'Cannot read properties of null (reading clearRect)'
        try {
  ...
  useEffect(() => {
    if (fabricCanvasRef.current && isCanvasReady && scale > 0) {
      ...
      // Don't call renderSlideElements() here - canvasScale state isn't updated yet!
  ...
  useEffect(() => {
    ...
    // elements would render at 0x0 or full 960x540 size, and async image loads
    // would capture stale positions that persist after the correct scale arrives.
    if (isCanvasReady && slide && fabricCanvasRef.current && canvasScale > 0) {
      renderSlideElements();
    }
  }, [canvasScale]);

  // Render slide elements when slide changes or elements are updated
  useEffect(() => {
    if (isCanvasReady && slide && canvasScale > 0) {
      renderSlideElements();
    }
  }, [slide, slide?.elements, isCanvasReady, presentationStyle]);

  // FIX: Clear inline video overlay when slide changes to prevent ghost videos
  useEffect(() => {
    setInlineVideoData(null);
  }, [slide?.id]);
  ...
  const renderSlideElements = useCallback(async () => {
    if (!fabricCanvasRef.current || !slide || !fabricModule || isDisposedRef.current) return;

    // Increment render ID to invalidate previous running renders
    const currentRenderId = ++renderIdRef.current;
    // Store current slide ID for async callbacks to check
    ...
    // 1. Update Background
    const bgColor = slide.backgroundColor || presentationStyle?.slideBackground || '#ffffff';
    ...
    if (slide.backgroundImage) {
      if (lastBgImageRef.current !== slide.backgroundImage) {
        ...
        const loadBg = (url, useCors = true) => {
          ...
          fabric.Image.fromURL(url, (img) => {
            ...
            if (img) {
              img.set({
                scaleX: (SLIDE_WIDTH * canvasScale) / img.width,
                scaleY: (SLIDE_HEIGHT * canvasScale) / img.height,
                selectable: false,
                evented: false,
              });
              ...
              rawImg.onload = () => {
                ...
                try {
                  ...
                  fImg.set({ scaleX: (SLIDE_WIDTH * canvasScale) / fImg.width, scaleY: (SLIDE_HEIGHT * canvasScale) / fImg.height, selectable: false, evented: false });
        ...
        const cachedBg = globalImageCache.get(slide.backgroundImage);
        if (cachedBg) {
          loadBg(cachedBg);
        } else {
          globalImageCache.fetchAndCache(slide.backgroundImage)
            .then(blobUrl => loadBg(blobUrl))
            .catch(() => loadBg(slide.backgroundImage));
        }
      }
    } else {
    ...
    // console.log(`📊 [CANVAS] Syncing ${sortedElements.length} elements for slide ${slide.id}:`,
    ...
    for (const element of sortedElements) {
      ...
      if (existing) {
        ...
        await renderElement(element, canvas, fabric, currentRenderId, currentSlideId, existing, isActive);
      } else {
        // Create new
        if (renderIdRef.current !== currentRenderId) return; // Abort if stale
        await renderElement(element, canvas, fabric, currentRenderId, currentSlideId);
      }
    }
    ...
    // console.log('🎨 [CANVAS] Render sync complete for cycle', currentRenderId, 'slideId:', slide?.id, 'hasCallback:', !!onRenderComplete);

    // Notify parent that canvas has finished rendering (with small delay for paint)
    if (onRenderComplete && slide?.id) {
      setTimeout(() => {
        // console.log('📸 [CANVAS] Calling onRenderComplete for slide:', slide?.id);
        onRenderComplete(slide?.id);
      }, 50);
    }
  }, [slide, canvasScale, presentationStyle, onRenderComplete]);
  ...
  const renderElement = async (element, canvas, fabric, renderId, currentSlideId, existingObj = null, skipLayout = false) => {
    ...
    // also applies to decks loaded from the database. Shallow-copy rather than
    // mutate — the original element stays intact in React state.
    if (isCardLikeShape(element, SLIDE_WIDTH, SLIDE_HEIGHT)) {
    ...
    const renderAsyncIcon = (targetIconName, targetRenderId) => {
      ...
      mapIconToPathAsync(targetIconName).then(({ path, svg, name }) => {
        // Allow late-arriving icons if still on same slide
        if (slideRef.current?.id !== currentSlideId) {
        ...
        if (svg) {
          ...
          fabric.loadSVGFromString(finalSvg, (objects, options) => {
            // Allow late-arriving icons if still on same slide
            if (slideRef.current?.id !== currentSlideId) return;
            if (objects && objects.length > 0) {
    ...
    switch (element.type) {
      case ELEMENT_TYPES.TEXT:
        const textStyle = presentationStyle?.textStyles?.[element.textType] || {};
        ...
        if (!textFillColor) {
          const bg = slide.backgroundColor || presentationStyle?.slideBackground || '#ffffff';
        ...
        // Check if this element was recently modified via toolbar (color picker, etc.)
        // If so, skip overwriting properties to prevent sync loop from reverting user's changes
        const isRecentlyModified = recentlyModifiedElementsRef.current.has(element.id);

        if (fabricObj && (fabricObj.type === 'textbox' || fabricObj.type === 'text')) {
          // SMART SYNC: Compare Fabric's current values with React state
          // If Fabric has different values AND we recently modified this element,
          // it means React state hasn't caught up yet - preserve Fabric's values
          if (isRecentlyModified) {
            console.log(`🛡️ [CANVAS] Recently modified text element ${element.id} - using smart sync`);

            // Only update position (layout), preserve all styling properties
            if (!skipLayout) {
              fabricObj.set({ left: scaledX, top: scaledY });
            }
            // Preserve width/height if changed
            if (scaledWidth && fabricObj.width !== scaledWidth) {
              fabricObj.set({ width: scaledWidth });
            }
            break; // Skip the rest of the text sync logic - preserve user's changes
          }

          // FONT PRESERVATION: If the Fabric object has a fontFamily that differs from what we're about to set,
          // it means the user just changed it via the font picker and React state hasn't updated yet.
          // Preserve the user's font choice to avoid the async timing issue.
          // NOTE: Only preserve if the element was recently modified by the user (toolbar interaction).
          // If not recently modified, React state is authoritative (e.g. AI edit changed the font).
          const currentFabricFont = fabricObj.fontFamily;
          const elementFont = element.fontFamily || textStyle.fontFamily || 'Inter, system-ui, sans-serif';

          // If Fabric has a different font than what element says, user might have just changed it - preserve it
          // But ONLY if this element was recently modified via toolbar - otherwise React state wins (AI edits, collab sync, etc.)
          if (currentFabricFont && currentFabricFont !== elementFont && !element.fontFamily && recentlyModifiedElementsRef.current.has(element.id)) {
            // User set font via picker, React state not synced yet - keep Fabric's font
            textOptions.fontFamily = currentFabricFont;
            textOptions.fontWeight = fabricObj.fontWeight || textOptions.fontWeight;
          }

          // FILL COLOR SYNC: Only preserve Fabric's fill over React state if the element was
          // recently modified by the user via toolbar (color picker, etc.).
          // If NOT recently modified, React state is authoritative - this allows AI edits,
          // collaboration sync, and other programmatic color changes to apply correctly.
          const fabricFill = fabricObj.fill;
          const elementFill = element.color || element.fill;
          if (fabricFill && elementFill && fabricFill !== elementFill) {
            if (recentlyModifiedElementsRef.current.has(element.id)) {
              // Element was recently modified via toolbar - keep Fabric's fill to prevent sync overwrite
              // console.log(`🎨 [CANVAS] Fill mismatch for ${element.id}: Fabric=${fabricFill}, React=${elementFill} - PRESERVING Fabric (recently modified)`);
              textOptions.fill = fabricFill;
            } else {
              // Element was NOT recently modified - React state is authoritative (AI edit, collab sync, etc.)
              // console.log(`🎨 [CANVAS] Fill mismatch for ${element.id}: Fabric=${fabricFill}, React=${elementFill} - APPLYING React state`);
              // textOptions.fill already has the correct React state value from textFillColor
            }
          }

          fabricObj.set(textOptions);

          // Apply per-character inline styles from element data (if any)
          // Only apply if the textbox is NOT currently being edited (to avoid overwriting live edits)
          if (element.styles && !fabricObj.isEditing && !isRecentlyModified) {
            fabricObj.set('styles', JSON.parse(JSON.stringify(element.styles)));
            fabricObj.set('dirty', true);
          }

          // NOTE: The old "FORCE COLOR OVERRIDE" that deleted all character-level fill styles
          // has been removed. Per-character styles are now intentional inline formatting from the user.
          // Global fill color changes from the top toolbar will set the whole-object fill but preserve
          // inline formatting unless explicitly overridden.
          // Update content if changed and not editing
          // CRITICAL: Also skip if this element was recently edited (prevents overwriting user's text)
          const isRecentlyEdited = recentlyEditedElementRef.current === element.id;
          if (element.content !== fabricObj.text && !fabricObj.isEditing && !isRecentlyEdited) {
            ...
            // console.log(`📝 [CANVAS] Skipping text sync for ${element.id} - recently edited`);
          }
        } else {
      ...
      case ELEMENT_TYPES.IMAGE:
        if (element.src) {
          ...
          if (fabricObj && isSameSource) {
            ...
            const loadAndRender = (finalSrc, useCors = true) => {
              ...
              fabric.Image.fromURL(finalSrc, (img) => {
                if (isDisposedRef.current || !fabricCanvasRef.current) return;
                // Allow late-arriving images if we're still on the same slide
                const stillSameSlide = slideRef.current?.id === currentSlideId;
                if (!stillSameSlide) {
                  console.log(`🚫 [CANVAS] Aborting image load for ${element.id} - slide changed`);
                  return;
                }
                ...
                if (img) {
                  ...
                  rawImg.onload = () => {
                    if (isDisposedRef.current || !fabricCanvasRef.current) return;
                    const stillSame = slideRef.current?.id === currentSlideId;
                    if (!stillSame) return;
          ...
          // Render a visible placeholder so the user sees something instead of a blank gap
      ...
      case ELEMENT_TYPES.SHAPE:
        ...
        if (complexShapes.includes(shapeType)) {
          // RECENTLY MODIFIED PROTECTION: Skip sync for shapes modified via toolbar
          if (recentlyModifiedElementsRef.current.has(element.id)) {
            console.log(`🛡️ [CANVAS] Skipping sync for recently modified shape element: ${element.id}`);
            break;
          }
          ...
          // RECENTLY MODIFIED PROTECTION: Skip sync for shapes modified via toolbar
          if (recentlyModifiedElementsRef.current.has(element.id)) {
            console.log(`🛡️ [CANVAS] Skipping sync for recently modified simple shape: ${element.id}`);
            break;
          }
      ...
      case ELEMENT_TYPES.ICON:
        ...
        // RECENTLY MODIFIED PROTECTION: Skip sync for icons modified via toolbar
        if (recentlyModifiedElementsRef.current.has(element.id)) {
          console.log(`🛡️ [CANVAS] Skipping sync for recently modified icon: ${element.id}`);
          break;
        }
        ...
        if (iconName && (!hasValidSvgSrc || isStaleCircleFallback)) {
          ...
          // Also check runtime iconCache (populated after first S3 fetch via cacheIconSVG)
          // This is handled below in the iconCache check
        }
        ...
        if (cachedPath) {
          ...
          try {
            ...
            // This prevents repeated S3 requests on every slide change
            fetch(svgUrl)
              .then(response => {
      ...
      case ELEMENT_TYPES.SVG_DIAGRAM: {
        ...
        // Uses fabric.loadSVGFromString → groupSVGElements scaled to slot dimensions.
        if (fabricObj) {
          canvas.remove(fabricObj);
          fabricObj = null;
        }
        ...
        if (!rawSvg) {
          // Placeholder when SVG hasn't been generated yet — show a loading-style hint
          // so users understand the diagram is being prepared (or invite them to generate).
        ...
        // Substitute currentColor with the resolved accent so themed colors apply pre-parse
        // Also inline-resolve any <style>/class= so the (fallback) fabric SVG parser
        // — which ignores CSS selectors — can also render correctly if used.
        ...
        // when it encounters features it doesn't understand, which is why complex
        // LLM-generated diagrams (multiple gradients + <marker> + <style>) only
        // partially render. The Break-apart flow still uses fabric's parser
        // separately, so interactivity is preserved on demand.
            ...
            fabric.Image.fromURL(
              ...
              (img) => {
                ...
                if (!img.width || !img.height) {
                  // SVG loaded but with no intrinsic size — happens when the
                  // SVG lacks width/height attrs AND the preprocessor couldn't
                  // derive them from viewBox. The fabric.Image would render
                  // invisibly. Show a fallback so the issue is visible.
        ...
        // Helper: render a fallback placeholder when both render paths fail.
        // User can still double-click → opens the regenerate modal.
        ...
        try {
          // PRIMARY: browser-native SVG render via data URL (handles every feature).
          ...
          fabric.loadSVGFromString(themedSvg, (objects, options) => {
            ...
            // Replace the local reference and add to canvas
            fabricObj = svgGroup;
            addToCanvasSorted(canvas, svgGroup, element.zIndex);
            canvas.requestRenderAll();
          });
      ...
      case ELEMENT_TYPES.EMBED:
        ...
        if (fabricObj) {
          ...
          // Create embed placeholder (static representation in edit mode)
  ...
  const getNextZIndex = useCallback(() => {
    ...
    const maxZ = Math.max(...slide.elements.map(e => parseInt(e.zIndex) || 0));
    return maxZ + 1;
  }, [slide?.elements]);

  // Local handlers
  const handleAddText = useCallback((textType = 'body', content = '', position = {}) => {
    console.log('[PresentationCanvas] Adding text:', textType);
    ...
    if (onAddElement) {
      onAddElement(slide.id, ELEMENT_TYPES.TEXT, newElement);
    } else {
      console.error('[PresentationCanvas] onAddElement prop is missing');
    }
  }, [slide, onAddElement, getNextZIndex]);
  ...
  const handleAddImage = useCallback((src = '', position = {}) => {
    console.log('[PresentationCanvas] Adding image:', src);
    ...
    if (onAddElement) {
      onAddElement(slide.id, ELEMENT_TYPES.IMAGE, newElement);
    } else {
      console.error('[PresentationCanvas] onAddElement prop is missing');
    }
  }, [slide, onAddElement, getNextZIndex]);
  ...
  const handleAddShape = useCallback((shapeConfig = 'rectangle', position = {}) => {
    ...
    console.log('[PresentationCanvas] Adding shape:', shapeType, shapeOptions);
    ...
    if (onAddElement) {
      onAddElement(slide.id, ELEMENT_TYPES.SHAPE, newElement);
    } else {
      console.error('[PresentationCanvas] onAddElement prop is missing');
    }
  ...
  const handleInsertIcon = useCallback((iconName) => {
    console.log('[PresentationCanvas] Inserting selected icon:', iconName);
    ...
    const firstIcon = slide?.elements?.find(e => e.type === ELEMENT_TYPES.ICON);
    ...
    if (firstIcon) {
      ...
      // Let's stick to true black #000000 if that's the strict request, or maybe presentationStyle.textPrimary if available?
    ...
    if (onAddElement) {
      onAddElement(slide.id, ELEMENT_TYPES.ICON, newElement);
    }
  }, [slide, onAddElement, getNextZIndex, presentationStyle]);
  ...
  const handleDeleteSelected = useCallback(() => {
    if (selectedElementId) {
      ...
      onDeleteElement(slide.id, selectedElementId);
      // Commit history
      if (handlersRef.current.saveToHistory) {
        handlersRef.current.saveToHistory();
      }
    }
  }, [slide, selectedElementId, onDeleteElement]);
  ...
  const handleImageFileSelect = useCallback((event) => {
    ...
    reader.onload = (e) => {
      ...
      console.log('[PresentationCanvas] Image loaded from file');
      ...
      if (fabricCanvasRef.current && typeof fabric !== 'undefined') {
        fabricModule.Image.fromURL(base64, (img) => {
          if (img) {
            ...
            // Also save to slide data
            if (onAddElement && slide) {
              onAddElement(slide.id, ELEMENT_TYPES.IMAGE, {
                src: base64,
                x: 100,
                y: 100,
  ...
  const handleAnimationVideoSelect = useCallback(async (event) => {
    ...
    console.log('[PresentationCanvas] Processing animation video:', file.name);
    ...
    try {
      ...
      if (onAddElement && slide) {
        onAddElement(slide.id, ELEMENT_TYPES.ANIMATION, {
  ...
  const handleDuplicate = useCallback(() => {
    ...
    const originalElement = slide.elements.find(el => el.id === activeObj.elementId);
    if (!originalElement) {
    ...
    if (handlersRef.current.onAddElement) {
      handlersRef.current.onAddElement(slide.id, originalElement.type, duplicateData);
    }
  }, []);
  ...
  const handleBringForward = useCallback(() => {
    ...
    if (activeObj && activeObj.elementId) {
      ...
      const slide = slideRef.current;
      if (slide?.elements && handlersRef.current.onUpdateElement) {
        ...
        handlersRef.current.onUpdateElement(slide.id, activeObj.elementId, { zIndex: newZ });
      }
    }
  }, []);
  ...
  const handleUpdateFillColor = useCallback((color) => {
    ...
    if (activeObj && activeObj.elementId && handlersRef.current.onUpdateElement) {
      // Mark element as recently modified to prevent sync loop from overwriting
      recentlyModifiedElementsRef.current.set(activeObj.elementId, Date.now());
      // Auto-clear after 500ms to allow React state to propagate
      setTimeout(() => {
        recentlyModifiedElementsRef.current.delete(activeObj.elementId);
      }, 500);
      ...
      handlersRef.current.onUpdateElement(slideRef.current.id, activeObj.elementId, updateProps);
      handlersRef.current.saveToHistory();
  ...
  const handleUpdateOpacity = useCallback((value) => {
    ...
    if (activeObj && activeObj.elementId && handlersRef.current.onUpdateElement) {
      ...
      handlersRef.current.onUpdateElement(slideRef.current.id, activeObj.elementId, { opacity: value });
      handlersRef.current.saveToHistory();
  ...
  const handleUpdateStrokeColor = useCallback((color) => {
    ...
    if (activeObj && activeObj.elementId && handlersRef.current.onUpdateElement) {
      ...
      handlersRef.current.onUpdateElement(slideRef.current.id, activeObj.elementId, strokeUpdateProps);
      handlersRef.current.saveToHistory();
  ...
  const handleUpdateFontSize = useCallback((size) => {
    ...
    if (activeObj && activeObj.elementId && handlersRef.current.onUpdateElement && (activeObj.type === 'textbox' || activeObj.type === 'i-text')) {
      // Mark element as recently modified to prevent sync loop from overwriting
      recentlyModifiedElementsRef.current.set(activeObj.elementId, Date.now());
      setTimeout(() => recentlyModifiedElementsRef.current.delete(activeObj.elementId), 500);
      ...
      handlersRef.current.onUpdateElement(slideRef.current.id, activeObj.elementId, { fontSize: size });
      handlersRef.current.saveToHistory();
  ...
  const handleUpdateLineHeight = useCallback((height) => {
    ...
    if (activeObj && activeObj.elementId && handlersRef.current.onUpdateElement && (activeObj.type === 'textbox' || activeObj.type === 'i-text')) {
      ...
      handlersRef.current.onUpdateElement(slideRef.current.id, activeObj.elementId, { lineHeight: newHeight });
      handlersRef.current.saveToHistory();
  ...
  const handleToggleBold = useCallback(() => {
    ...
    if (activeObj && activeObj.elementId && handlersRef.current.onUpdateElement && (activeObj.type === 'textbox' || activeObj.type === 'i-text')) {
      ...
      handlersRef.current.onUpdateElement(slideRef.current.id, activeObj.elementId, { fontWeight: newWeight });
      handlersRef.current.saveToHistory();
  ...
  const handleToggleItalic = useCallback(() => {
    ...
    if (activeObj && activeObj.elementId && handlersRef.current.onUpdateElement && (activeObj.type === 'textbox' || activeObj.type === 'i-text')) {
      ...
      handlersRef.current.onUpdateElement(slideRef.current.id, activeObj.elementId, { fontStyle: newStyle });
      handlersRef.current.saveToHistory();
  ...
  const applyInlineOrWholeStyle = useCallback((styleKey, newValue, elementUpdateKey) => {
    ...
    if (activeObj.isEditing && activeObj.selectionStart !== activeObj.selectionEnd) {
      ...
      const styleObj = {};
      // For fontSize, convert from logical to canvas-scaled
      ...
      handlersRef.current.onUpdateElement(slideRef.current.id, activeObj.elementId, updatePayload);
    }
    handlersRef.current.saveToHistory();
  }, []);
  ...
  const flushPendingEditsToHistory = useCallback(() => {
    if (!slideRef.current?.id || slideRef.current.id !== currentSlideIdRef.current) return;
    // Skip if an undo/redo is in progress to avoid corrupting history
    if (isUndoRedoActiveFor(slideRef.current.id)) return;

    // Clear any pending debounce for this slide
    const pending = historyDebounceMapRef.current.get(slideRef.current.id);
    if (pending) {
      clearTimeout(pending);
      historyDebounceMapRef.current.delete(slideRef.current.id);
    }

    if (!slideRef.current?.id || !slideRef.current?.elements) return;
    const history = getSlideHistory(slideRef.current.id);
    if (!history) return;
    ...
    if (preEditSnapshotRef.current) {
      ...
      if (snapshotStr !== history.lastSaved) {
        ...
        prevElementsMapRef.current.set(slideRef.current.id, snapshotStr);
        console.log('💾 [FLUSH] Saved pre-edit snapshot for slide', slideRef.current.id, 'index:', history.index);
      }
      preEditSnapshotRef.current = null;
    }

    // Do NOT snapshot current fabric state here; leaving it caused duplicate undo steps.
  }, [getSlideHistory, isUndoRedoActiveFor]);

  // Undo - restore previous state for current slide
  const handleUndo = useCallback(() => {
    if (!slide?.id) return;
    if (slide.id !== currentSlideIdRef.current) return; // guard per-slide history
    ...
    console.log('⏪ [UNDO] Called for slide', slide.id, 'index:', history.index, 'history length:', history.states.length);
    if (history.index <= 0) {
      console.log('⏪ [UNDO] Nothing to undo for slide', slide.id);
      return;
    }

    setUndoRedoActiveFor(slide.id, true);
    history.index -= 1;
    ...
    if (prevState) {
      ...
      console.log('⏪ [UNDO] Restoring state with', elementsToRestore.length, 'elements for slide', slide.id);
      const restoredSnapshotStr = JSON.stringify(normalizedState);
      prevElementsMapRef.current.set(slide.id, restoredSnapshotStr);
      ...
      fabricCanvasRef.current?.discardActiveObject();
      onUpdateElement(slide.id, null, { elements: elementsToRestore });
      if (backgroundToRestore !== undefined && backgroundToRestore !== slide.backgroundColor && onUpdateSlideBackground) {
        onUpdateSlideBackground(backgroundToRestore);
      }
    }

    setTimeout(() => {
      setUndoRedoActiveFor(slide.id, false);
    }, 200);
  }, [slide?.id, onUpdateElement, getSlideHistory, flushPendingEditsToHistory, setUndoRedoActiveFor]);

  // Redo - restore next state for current slide
  const handleRedo = useCallback(() => {
    ...
    console.log('⏩ [REDO] Called for slide', slide.id, 'index:', history.index, 'history length:', history.states.length);
    if (history.index >= history.states.length - 1) {
      console.log('⏩ [REDO] Nothing to redo for slide', slide.id);
      return;
    }
    ...
    if (nextState) {
      ...
      console.log('⏩ [REDO] Restoring state with', elementsToRestore.length, 'elements for slide', slide.id);
      const restoredSnapshotStr = JSON.stringify(normalizedState);
      prevElementsMapRef.current.set(slide.id, restoredSnapshotStr);
      history.lastSaved = restoredSnapshotStr;
      // Discard active selection so renderSlideElements doesn't skip layout (skipLayout=isActive)
  ...
  const capturePreEditSnapshot = useCallback(() => {
    if (isUndoRedoActiveFor(slide?.id) || !slide?.elements) return;
    ...
    if (!preEditSnapshotRef.current) {
      ...
      console.log('📷 [HISTORY] Captured pre-edit snapshot with', slide.elements?.length || 0, 'elements for slide', slide.id);
    }
  }, [slide?.id, slide?.elements]);

  // Commit the pre-edit snapshot to history (call this after edit is complete)
  const commitToHistory = useCallback(() => {
    if (isUndoRedoActiveFor(slide?.id) || !slide?.id) return;

    // Clear any pending debounce for this slide
    const pending = historyDebounceMapRef.current.get(slide.id);
    if (pending) {
      clearTimeout(pending);
      historyDebounceMapRef.current.delete(slide.id);
    }
    ...
    if (snapshot) {
      ...
      if (snapshotStr !== history.lastSaved) {
        ...
        history.index = history.states.length - 1;
        history.lastSaved = snapshotStr;
        prevElementsMapRef.current.set(slide.id, snapshotStr);
        ...
        console.log('💾 [HISTORY] Committed pre-edit state for slide', slide.id, 'index:', history.index, 'elements:', snapshot.elements?.length || 0);
      }

      preEditSnapshotRef.current = null;
    }
  }, [slide?.id, getSlideHistory]);

  // Save state to history with debouncing for rapid edits
  const saveToHistory = useCallback(() => {
    if (isUndoRedoActiveFor(slideRef.current?.id) || !slideRef.current?.id) return;

    const history = getSlideHistory(slideRef.current.id);
    if (!history) return;

    const currentState = collectCurrentSlideState(slideRef.current.id);
    if (!currentState) return;
    const currentStr = JSON.stringify(currentState);

    // Skip if state hasn't changed
    if (currentStr === history.lastSaved) {
      console.log('⏭️ [HISTORY] Skipped duplicate state for slide', slideRef.current.id);
      return;
    }
    ...
    history.index = history.states.length - 1;
    history.lastSaved = currentStr;
    prevElementsMapRef.current.set(slideRef.current.id, currentStr);
    ...
    // console.log('💾 [HISTORY] Saved state for slide', slideRef.current.id, 'index:', history.index, 'elements:', currentState.elements?.length || 0);
  }, [collectCurrentSlideState, getSlideHistory]);
  ...
  const debouncedSaveToHistory = useCallback(() => {
    ...
    // Schedule save after 300ms of inactivity for this slide
    const timer = setTimeout(() => {
      saveToHistory();
      historyDebounceMapRef.current.delete(slideRef.current?.id);
    }, 300);
    if (slideRef.current?.id) {
      historyDebounceMapRef.current.set(slideRef.current.id, timer);
    }
  }, [saveToHistory]);
  ...
  useEffect(() => {
    ...
    handlersRef.current = {
      ...
      handleBringForward,
      handleSendBackward,
      setInlineToolbar: (state) => setInlineToolbar(state),
      capturePreEditSnapshot: () => {
        // Capture snapshot immediately using current slide state
        if (!preEditSnapshotRef.current && slideRef.current) {
          preEditSnapshotRef.current = JSON.parse(JSON.stringify({
            elements: slideRef.current.elements || [],
            backgroundColor: slideRef.current.backgroundColor,
          }));
          console.log('📷 [HANDLERS] Captured pre-edit snapshot with', slideRef.current.elements?.length || 0, 'elements');
        }
      },
      saveToHistory: () => {
        ...
        if (!isUndoRedoActiveFor(slideRef.current?.id) && slideRef.current?.id) {
          const history = getSlideHistory(slideRef.current.id);
          if (!history) return;
          ...
          if (currentStr !== history.lastSaved) {
            ...
            console.log('💾 [HANDLERS] Saved post-edit state for slide', slideRef.current.id, 'index:', history.index);
          }
        }
      },
      debouncedSaveToHistory: () => {
        ...
        const timer = setTimeout(() => {
          if (!isUndoRedoActiveFor(slideRef.current?.id) && slideRef.current?.id && slideRef.current?.elements) {
            // First commit any pending pre-edit snapshot
            commitToHistory();
            ...
            if (currentStr !== history.lastSaved) {
              ...
              console.log('💾 [HANDLERS-DEBOUNCED] Saved state for slide', slideRef.current.id, 'index:', history.index);
            }
          }
          if (slideRef.current?.id) {
            historyDebounceMapRef.current.delete(slideRef.current.id);
  ...
  }, [slide, onUpdateElement, onDeleteElement, saveToHistory, commitToHistory, debouncedSaveToHistory, canvasScale, getSlideHistory]);
  ...
  const handleTextAlign = useCallback((align) => {
    ...
    if (activeObj && activeObj.elementId && handlersRef.current.onUpdateElement && (activeObj.type === 'textbox' || activeObj.type === 'i-text')) {
      ...
      handlersRef.current.onUpdateElement(slideRef.current.id, activeObj.elementId, { textAlign: align });
      handlersRef.current.saveToHistory();
  ...
  const handleApplyFontCombination = useCallback((combination) => {
    ...
    const element = slide?.elements?.find(el => el.id === activeObj.elementId);
    ...
    // Update element in slide state - use onUpdateElement prop directly with slide.id from prop
    // This ensures we're not using stale refs and the update is properly applied
    if (onUpdateElement && activeObj.elementId && slide?.id) {
      console.log('🎨 [FONT] Updating element', activeObj.elementId, 'on slide', slide.id, 'with font:', fontToApply.family);
      onUpdateElement(slide.id, activeObj.elementId, {
        fontFamily: fontToApply.family,
        fontWeight: fontToApply.weight,
      });
      console.log('✅ [FONT] Applied font combination:', combination.name, 'to element', activeObj.elementId);
    } else {
      console.error('❌ [FONT] Failed to update element - missing:', { onUpdateElement: !!onUpdateElement, elementId: activeObj.elementId, slideId: slide?.id });
    }
  }, [slide, onUpdateElement]);
  ...
  useImperativeHandle(ref, () => ({
    ...
    openModal: (name) => {
      switch (name) {
        ...
        case 'animation': setShowAnimationModal(true); break;
      }
    },
    ...
    // pass. Uses multiplier 1 (= native 960x540) to keep the screenshot
    // under ~150 KB so the vision model accepts it. Returns null if the
    // canvas isn't ready or the export tainted (e.g. cross-origin images).
    snapshotForCritique: () => {
      ...
      try {
        ...
        console.warn('🖼️ [CRITIQUE] canvas snapshot failed (tainted?):', err);
        return null;
      }
    },
    ...
    addChart: async (chartConfig) => {
      ...
      try {
        ...
        if (fabricRef?.Chart) {
          ...
          const chartLeft = SLIDE_WIDTH / 2 - chartWidth / 2;
          const chartTop = SLIDE_HEIGHT / 2 - chartHeight / 2;

          onAddElement(slide.id, 'chart', {
            x: chartLeft,
            y: chartTop,
            width: chartWidth,
            height: chartHeight,
          ...
          return new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
              ...
              fabricModule.Image.fromURL(dataUrl, (img) => {
                ...
                img.set({
                  left: SLIDE_WIDTH / 2 - 240,
                  top: SLIDE_HEIGHT / 2 - 160,
                  scaleX: 0.8,
                  scaleY: 0.8,
                });
      ...
      return new Promise((resolve, reject) => {
        try {
          fabricModule.Image.fromURL(diagramImage, (img) => {
            ...
            const maxHeight = SLIDE_HEIGHT * 0.8;
            ...
            img.set({
              left: SLIDE_WIDTH / 2 - finalWidth / 2,
              top: SLIDE_HEIGHT / 2 - finalHeight / 2,
              scaleX: scale,
              scaleY: scale,
            });
  ...
  }), [handleAddText, handleAddImage, handleAddShape, handleDeleteSelected, handleToDataURL, handleToJSON, handleZoomIn, handleZoomOut, handleResetZoom, slide, onAddElement, handleToggleBold, handleToggleItalic, handleUpdateFillColor, handleUpdateStrokeColor, handleUpdateFontSize, handleUpdateLineHeight, handleTextAlign, handleUndo, handleRedo, handleDuplicate, handleBringForward, handleSendBackward, handleAddElement, fillColor, strokeColor, fontSize, lineHeight]);
            ...
            {element.type === ELEMENT_TYPES.TEXT && (
              editingTextId === element.id ? (
                <TextInput
                  ...
                  onBlur={() => {
                    onUpdateElement(slide.id, element.id, { content: editingTextValue });
  ...
  return (
    <View style={[styles.container, { backgroundColor: isEditable ? theme.background : 'transparent' }]} ref={containerRef}>
      {/* Toolbar */}
        ...
        <View style={[styles.toolbar, isMobile && { paddingHorizontal: 8, paddingVertical: 6 }]}>
          ...
          {isMobile ? (
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 4, rowGap: 6, alignItems: 'center', zIndex: 2000 }}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    if (formatPainterActive) {
                      ...
                      const canvas = fabricCanvasRef.current;
                      const activeObj = canvas?.getActiveObject();
                      if (activeObj?.elementId && slide?.elements) {
                        const element = slide.elements.find(el => el.id === activeObj.elementId);
              ...
              <View style={{ zIndex: 1100 }}>
                <ColorPickerDropdown label="Bg" value={slide?.backgroundColor || '#ffffff'} onChange={(c) => onUpdateSlideBackground && onUpdateSlideBackground(c)} themeColors={['#ffffff', '#f5f5f5', '#1a1a2e']} theme={{ ...theme, surface: '#ffffff' }} compact={true} />
              </View>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 2, rowGap: 4, zIndex: 2000 }}>
                ...
                {!isMobile && (
                  <>
                    ...
                    <Tooltip text="Save Presentation" theme={theme}>
                    ...
                    {false && presentationId && onShowCollaboration && (
                    ...
                    {presentationId && onShowAnalytics && (
                    ...
                    <Tooltip text="Export Presentation" theme={theme}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 2, rowGap: 4, zIndex: 1000, marginTop: 4, paddingTop: 4, borderTopWidth: 1, borderTopColor: '#f0f0f0' }}>
                ...
                <View style={{ zIndex: 1100 }}>
                  <ColorPickerDropdown
                    ...
                    onChange={(c) => onUpdateSlideBackground && onUpdateSlideBackground(c)}
              ...
              reader.onload = (event) => {
                ...
                tempVideo.onloadedmetadata = () => {
                  ...
                  const addElementWithThumbnail = (thumbnail) => {
                    if (slide?.id) {
                      const maxZ = slide.elements?.length > 0
                        ? Math.max(...slide.elements.map(el => parseInt(el.zIndex) || 0)) + 1
                        : 1;
                      ...
                      console.log('❌ [VIDEO_UPLOAD] No slide ID available:', slide);
                    }
                  };
              ...
              >
                ...
                <InlineVideoOverlay
                  ...
                  onClose={() => {
                    ...
                    if (inlineVideoData?.id && slide?.id && onDeleteElement) {
                      onDeleteElement(slide.id, inlineVideoData.id);
                    }
                    setInlineVideoData(null);
                  }}
      ...
      {/* Slide info - hidden in present mode */}
      {isEditable && (
        <View style={[styles.slideInfo, { backgroundColor: theme.surface, borderTopColor: theme.border }, isMobile && { justifyContent: 'center', gap: 8 }]}>
          <Text style={[styles.slideInfoText, { color: theme.textSecondary }, isMobile && { textAlign: 'center' }]}>
            Slide {slide?.order || 1} • {slide?.title || 'Untitled'}
          </Text>
          {!isMobile && (
            <Text style={[styles.slideInfoText, { color: theme.textSecondary }]}>
      ...
      <TablePickerModal
        ...
        onInsert={(rows, cols, includeHeader, themeColors) => {
          ...
          // Center in canvas
          const x = (SLIDE_WIDTH - tableWidth) / 2;
          const y = (SLIDE_HEIGHT - tableHeight) / 2;
          ...
          // Add to slide via onAddElement prop
          if (onAddElement && slide?.id) {
            onAddElement(slide.id, 'table', tableElement);
            ...
            if (onAddElement && slide) {
              ...
              recVideo.onloadedmetadata = () => {
                ...
                recVideo.onseeked = () => {
                  ...
                  onAddElement(slide.id, ELEMENT_TYPES.VIDEO, {
                    src: videoData.src,
                    thumbnail: thumbnail,
                    videoType: 'recorded',
      ...
      <EmbedSourceModal
        ...
        onSelectEmbed={(embedData) => {
          ...
          if (onAddElement && slide) {
            onAddElement(slide.id, ELEMENT_TYPES.EMBED, {
              src: embedData.src,
      ...
      <FormsButtonModal
        ...
        onSelectButton={(buttonData) => {
          ...
          if (onAddElement && slide) {
            onAddElement(slide.id, ELEMENT_TYPES.BUTTON, {
