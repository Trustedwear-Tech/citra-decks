 ...
 * 
 * Displays analytics for a shared presentation including:
 * - Total views and unique viewers
 * - Average time spent and completion rate
 * - Slide-by-slide engagement heatmap
 * - Recent viewer activity
 */
const PresentationAnalyticsModal = ({
    visible,
    onClose,
    presentationId,
    presentationTitle,
    ...
    const fetchAnalytics = async () => {
        ...
        try {
            ...
            const response = await fetch(
                `${apiBaseUrl}/api/analytics/presentation/${presentationId}?days=30`,
                { headers }
            );
            ...
            setError('Could not load analytics. Make sure the presentation has been shared.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const toggleAnalytics = async (enabled) => {
        ...
        try {
            const headers = getAuthHeaders ? await getAuthHeaders() : {};
            const response = await fetch(
                `${apiBaseUrl}/api/analytics/presentation/${presentationId}/toggle?enabled=${enabled}`,
                { method: 'POST', headers }
            );
    ...
    const maxDuration = analytics?.slide_stats?.length > 0
        ? Math.max(...analytics.slide_stats.map(s => s.avg_duration_ms))
        : 1;
                ...
                <View style={styles.container}>
                    ...
                    <View style={styles.header}>
                        <View style={styles.headerLeft}>
                            ...
                            <Text style={styles.headerTitle} numberOfLines={1}>Presentation Analytics</Text>
                        </View>
                            ...
                            <>
                                ...
                                <View style={styles.toggleRow}>
                                    <View style={{ flex: 1 }}>
                                        ...
                                        <Text style={styles.toggleHint}>Track views when presentation is shared</Text>
                                    </View>
                                    <Switch
                                ...
                                <View style={styles.section}>
                                    <Text style={styles.sectionTitle}>
                                        <Ionicons name="bar-chart-outline" size={isMobile ? 15 : 18} color={theme.text} /> Slide Engagement
                                    </Text>

                                    {analytics?.slide_stats?.map((slide, index) => {
                                        const barWidth = maxDuration > 0
                                            ? (slide.avg_duration_ms / maxDuration) * 100
                                            : 0;
                                        const isDropOff = slide.drop_off_count > 0;

                                        return (
                                            <View key={index} style={styles.slideRow}>
                                                <Text style={styles.slideLabel}>Slide {index + 1}</Text>
                                                ...
                                                <Text style={styles.slideDuration}>
                                                    {formatDuration(slide.avg_duration_ms)}
                                                </Text>
                                                {isDropOff && (
                                                    <View style={styles.dropOffBadge}>
                                                        <Text style={styles.dropOffText}>⚠️ {slide.drop_off_count}</Text>
                                                    </View>
                                                )}
                                            </View>
                                        );
                                    ...
                                    {(!analytics?.slide_stats || analytics.slide_stats.length === 0) && (
                                        <Text style={styles.noDataText}>No slide data available yet</Text>
                                    )}
                                </View>
                                ...
                                <View style={styles.section}>
                                        ...
                                        <View key={index} style={styles.viewerRow}>
                                            ...
                                            <View style={styles.viewerInfo}>
                                                ...
                                                <Text style={styles.viewerMeta} numberOfLines={1}>
                                                    {formatTimeAgo(viewer.viewed_at)} • {formatDuration(viewer.duration_ms)} • {viewer.slides_viewed} slides
                                                </Text>
                                            </View>
                                        </View>
                                    ))}
                                    ...
                                    {(!analytics?.recent_viewers || analytics.recent_viewers.length === 0) && (
                                        <Text style={styles.noDataText}>No viewers yet. Share your presentation to start tracking.</Text>
                                    )}
                                </View>
                            </>
                        )}
...
export default PresentationAnalyticsModal;
