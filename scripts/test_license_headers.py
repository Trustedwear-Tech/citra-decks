...
def test_generated_and_vendored_paths_are_excluded():
    # The vendored Citra Common packages stay Apache-2.0 (see NOTICE).
    assert lh.is_excluded("citra-auth/citra_auth/__init__.py")
    assert lh.is_excluded("citra-cache/pyproject.toml")
    assert lh.is_excluded("citra-llm/citra_llm/client.py")
    assert lh.is_excluded("citra-mongo/README.md")
    assert lh.is_excluded("citra-queue/tests/test_queue.py")
    assert lh.is_excluded("citra-service-utils/setup.py")
    assert lh.is_excluded("ui/node_modules/x/index.js")
    assert lh.is_excluded("x/dist/bundle.js")
    assert lh.is_excluded("a/b.min.js")
    assert not lh.is_excluded("main.py")
    # Only a top-level citra-* package dir is vendored, not any path that
    # happens to contain the substring.
    assert not lh.is_excluded("services/citra-auth-proxy.py")
