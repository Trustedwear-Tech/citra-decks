...
for _ in $(seq 1 60); do
  if $COMPOSE exec -T mongodb mongosh --quiet -u root -p "$MONGO_PW" --authenticationDatabase admin \
       --eval "rs.status().ok" 2>/dev/null | grep -q '^1$'; then
    echo "   [ok] Mongo replica set ready"; MONGO_OK=true; break
  fi
  sleep 3
done
...
if ! docker run --rm --network citra-decks-network --entrypoint sh minio/mc:latest -c \
  ...
  echo "          exists. Create it in the MinIO console at http://localhost:9003" >&2
