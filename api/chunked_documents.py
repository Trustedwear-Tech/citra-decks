...
#     topic: str = Form(None),
#     create_embeddings: bool = Form(True),  # NEW: Option to create embeddings
#     file: UploadFile = File(...),
#     service: EnhancedChunkedDocumentService = Depends(get_chunked_service)
# ):
#     """Upload and process a document with optional embedding creation"""
#     try:
...
#         logger.info(f"🔒 Processing upload for authenticated user_id: {user_id}, create_embeddings: {create_embeddings}")
...
#         try:
#             # Use the enhanced service's complete pipeline
#             document_id = str(uuid.uuid4())
            
#             metadata = await service.store_document_with_embeddings(
...
#             return {
#                 "message": f"Document uploaded and processed successfully{' with embeddings' if create_embeddings else ''}",
#                 "document_id": document_id,
